...
```bash
# Baseline supervisor
uv run dao-ai validate -c examples/99_complete_applications/hardware_store/hardware_store.yaml

# Instructed retrieval
uv run dao-ai validate -c examples/99_complete_applications/hardware_store/hardware_store_instructed.yaml

# Swarm
uv run dao-ai validate -c examples/99_complete_applications/hardware_store/hardware_store_swarm.yaml

# Lakebase memory
uv run dao-ai validate -c examples/99_complete_applications/hardware_store/hardware_store_lakebase.yaml
```
...
uv run dao-ai chat -c examples/99_complete_applications/hardware_store/hardware_store_swarm.yaml

# Render the multi-agent graph to an image
uv run dao-ai graph -c examples/99_complete_applications/hardware_store/hardware_store.yaml -o hardware_store_architecture.png
```
...
uv run dao-ai workflow up \
  -c examples/99_complete_applications/hardware_store/hardware_store.yaml \
  -p DEFAULT
```
...
- **Lakebase persistent memory + background extraction** — `hardware_store_lakebase.yaml`; parallels the commerce swarm memory model in `examples/99_complete_applications/commerce/`
