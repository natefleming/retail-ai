...
DATABRICKS_CONFIG_PROFILE=DEFAULT uv run dao-ai validate \
  -c examples/99_complete_applications/quick_serve_restaurant/quick_serve_restaurant.yaml
...
- **Durable Lakebase memory (the upgrade path)** — `examples/99_complete_applications/hardware_store_lakebase.yaml`
