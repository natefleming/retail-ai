...
| 🛡️ **Unity AI Gateway** | `use_ai_gateway: true` on every chat endpoint — uniform governance, usage tracking, rate-limit pooling. Sonnet primary with a Sonnet-4-6 fallback. |
...
DATABRICKS_CONFIG_PROFILE=DEFAULT uv run dao-ai validate \
  -c examples/99_complete_applications/loyalty_offer_personalization/loyalty_offer_personalization.yaml
```
...
- **Supervisor orchestration** — `app.orchestration.supervisor` in this config; contrast the pipeline variant in `examples/99_complete_applications/commerce/commerce_supervisor.README.md`.
- **Lakebase memory** — `examples/99_complete_applications/hardware_store_lakebase.yaml`.
