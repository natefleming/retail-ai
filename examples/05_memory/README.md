...
```bash
# In-memory (testing)
dao-ai chat -c examples/05_memory/in_memory_basic.yaml \
  --thread-id my_session

# PostgreSQL (production)
dao-ai chat -c examples/05_memory/postgres_persistence.yaml \
  --thread-id user_123

# Lakebase (Databricks-native)
dao-ai chat -c examples/05_memory/lakebase_persistence.yaml \
  --thread-id user_123
```
