...
uv run python examples/20_a2a_protocol/client/client.py \
    --base-url "$APP_URL" \
    --bearer-token "$TOKEN"
```
...
DAO_AI_CONFIG_PATH=examples/20_a2a_protocol/a2a_minimal.yaml \
    uv run python -m dao_ai.apps.server &

uv run python examples/20_a2a_protocol/client/client.py --base-url http://localhost:8000
```
...
step. To exercise HITL end-to-end, swap in `examples/07_human_in_the_loop/human_in_the_loop.yaml`
