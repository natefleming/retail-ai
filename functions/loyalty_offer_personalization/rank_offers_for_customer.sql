...
RETURN (
  -- ai_query returns the JSON for the *value* of the responseFormat's
  -- outermost field (it strips the outer "ranking" wrapper before
  -- returning). So we parse with the inner shape: STRUCT<offers: ARRAY<...>>.
  SELECT any_value(from_json(
    resp.r,
    'STRUCT<offers: ARRAY<STRUCT<offer_id: STRING, rank: INT, score: DOUBLE, reason: STRING>>>'
  ).offers)
  FROM (
    SELECT ai_query(
