...
READS SQL DATA
COMMENT 'Retrieves detailed inventory information for specific products by SKU at a specific store location. Both store and sku are required. For broad inventory queries without specific SKUs, use the Genie analytics tool instead.'
RETURN 
SELECT 
  inventory_id
  ,products.sku
