    ...
    def _create_table_if_not_exists(self) -> None:
        ...
        with self._pool.connection() as conn:
            with conn.cursor() as cur:
                ...
                # Detect Lakebase orphaned-table situation eagerly so operators
                # see a clear diagnostic at boot instead of cryptic
                # "permission denied for sequence ..._id_seq" errors on the
                # first cache write. See docs/mcp_server.md troubleshooting.
                self._diagnose_table_ownership(cur)

    def _diagnose_table_ownership(self, cur: Any) -> None:
        """Detect the Lakebase 'orphaned cache table' failure mode.

        The dao-ai semantic-cache table is designed to be shared across Genie
        spaces and across multiple apps on the same Lakebase project — rows
        are isolated via the ``genie_space_id`` column. But Lakebase enforces
        NOINHERIT on the ``DATABRICKS_SUPERUSER`` role, so when a *prior* App
        SP created the cache tables and the App is later renamed (or
        redeployed under a new SP), the **new** SP can't take ownership of
        the orphaned tables, can't ``ALTER TABLE ... OWNER``, and ``GRANT``
        from any non-owner identity silently no-ops on the underlying
        sequence's ACL. The first cache *write* then fails with
        ``permission denied for sequence <name>_id_seq``.

        Detecting this at table-create time gives the operator a clear,
        actionable diagnostic instead of leaving them to interpret an
        opaque permission error on the first inference request.
        """
        try:
            cur.execute(
                """
        ...
        if hasattr(row, "get"):
            can_insert = row.get("can_insert")
            owner = row.get("owner")
            me = row.get("me")
        else:
            can_insert, owner, me = row

        if can_insert and owner == me:
            return

        # Even if can_insert is True (because we granted PUBLIC INSERT or
        # similar), a different owner still blocks sequence-level writes for
        # the SERIAL primary key — surface this clearly.
        message = (
            f"Postgres semantic cache table {self.table_name!r} exists in this "
            f"Lakebase project but is owned by role {owner!r}, not the current "
            f"connection user {me!r}. The dao-ai cache table is multi-tenant "
            f"(rows are isolated by genie_space_id), so multiple Genie spaces "
            "and apps are designed to share one table — but Lakebase enforces "
            "NOINHERIT on DATABRICKS_SUPERUSER, so a new App SP cannot take "
            "over ownership of a prior SP's tables and writes will fail with "
            "'permission denied for sequence <name>_id_seq' even when "
            "table-level GRANTs appear to succeed. Remediation:\n"
            f"  (preferred) Have a Lakebase project owner / cloud_admin run:\n"
            f"    DROP TABLE IF EXISTS {self.table_name} CASCADE;\n"
            f"    DROP TABLE IF EXISTS {self.prompt_history_table} CASCADE;\n"
            f"  The next App boot will recreate them under the current SP "
            "and writes will succeed.\n"
            "  (emergency unblock) Set distinct table_name + prompt_history_table "
            "on context_aware_cache_parameters (e.g. <app_name>_cache / "
            "<app_name>_prompt_history) so the current SP creates fresh tables "
            "it owns. Trades table sprawl for not needing project-owner access. "
            "See docs/mcp_server.md troubleshooting for details."
        )
        logger.error(
            "mcp.semantic_cache.orphaned_table",
            layer=self.name,
            table=self.table_name,
            owner=owner,
            current_user=me,
