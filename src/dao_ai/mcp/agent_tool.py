...
Conversation continuity: ``invoke_agent`` accepts a ``conversation_id``
(or ``thread_id`` alias) as a tool argument, or ``_meta.conversation_id``
on the ``tools/call`` request, or ``X-Databricks-Conversation-Id`` on the
HTTP transport. If none is supplied the agent generates a UUID; the
resolved id is echoed on both the structured response and
...
class AgentInvocationResult(BaseModel):
    ...
    conversation_id: Optional[str] = Field(
        default=None,
        description=(
            "Resolved conversation key — either client-supplied (via the "
            "``conversation_id`` / ``thread_id`` tool arg, ``_meta.conversation_id`` "
            "on the ``tools/call`` request, or the ``X-Databricks-Conversation-Id`` "
            "header) or generated by the agent when none was supplied. Pass "
            "back on the next call to resume state against a Lakebase-backed "
            "checkpointer."
        ),
    )
...
def _resolve_conversation_id(
    *,
    conversation_id_arg: Optional[str],
    thread_id_arg: Optional[str],
    ...
    Precedence (highest first):
      1. ``conversation_id`` tool arg
      2. ``thread_id`` tool arg (LangGraph-native alias)
      3. ``_meta.conversation_id`` on the ``tools/call`` request
      4. ``X-Databricks-Conversation-Id`` HTTP header
      5. None — agent generates a UUID downstream

    Returns ``(resolved_id, source)`` where ``source`` is one of
    ``"arg"``, ``"meta"``, ``"header"``, or ``None`` when nothing was
    supplied and the agent will generate one.
    ...
    if conversation_id_arg:
        return conversation_id_arg, "arg"
    if thread_id_arg:
        return thread_id_arg, "arg"
    if meta is not None:
...
def register_agent_as_tool(mcp: FastMCP, config: AppConfig) -> str:
    ...
    async def invoke_agent(
        ...
        To maintain a multi-turn conversation, supply a stable
        ``conversation_id`` (or the ``thread_id`` alias) on each call, or
        set ``_meta.conversation_id`` on the ``tools/call`` request, or set
        the ``X-Databricks-Conversation-Id`` HTTP header. If none is
        supplied the agent generates a UUID; the resolved id is echoed on
        ``AgentInvocationResult.conversation_id`` and ``_meta.conversation_id``
        so the caller can persist it and reuse it on the next turn.
        ...
        logger.info(
            ...
            # ``arg`` | ``meta`` | ``header`` | None → None means the agent
            # will generate a fresh UUID and this is a stateless call.
