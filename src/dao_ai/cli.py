...
# the config fingerprint plus a per-file SHA-256 registry of the files dao-ai
# generated, so a later deploy can tell a hand-edited generated file from an
# untouched one without relying on mtimes (which mtime-preserving copies —
# shutil.copy2, cp -p, git checkout — silently defeat). DABs/Terraform-style
# state file; intentionally not gitignored.
_STAGING_MANIFEST = ".dao-ai-manifest.yaml"
_MANIFEST_VERSION = 1

# Legacy marker (pre-manifest): its CONTENT was the config fingerprint and its
# MTIME the "last generated" timestamp. Read-only fallback for dirs generated by
...
def _write_staging_manifest(
    ...
    *,
    ...
    LAST, after the writer. Records:

    * ``config_fingerprint`` (from :func:`_config_fingerprint`) — the source
      config the bundle was generated from; :func:`_staged_config_is_stale` reads
      it back to detect config drift. App bundles pass it; the workflow bundle
    ...
    * ``tracked`` — the full set of non-ignored paths on disk right now (generated
      files + copied user code). A later file absent from ``tracked`` is a
...
def _staging_dir_has_local_edits(bundle_dir: Path) -> bool:
    ...
    * a non-ignored file on disk that was NOT present at generate time
      (``tracked``) — a user-added stray (e.g. a hand-dropped
      ``resources/jobs.yml``). Build noise (``.venv``/``__pycache__``/``dist``/…)
      is excluded via :func:`_is_ignored_staging_path` so it never counts.

    Editing an existing NON-generated file (user code that was in ``tracked``) is
    ...
    edit. Superseded once the dir is re-staged with a manifest.

    Neither present: treat as edited (a user-created or unknown dir).
    """
    ...
    if manifest is not None:
        ...
        # key skip stray detection rather than flag every user file —
        # conservative, and superseded on re-stage.
...
def _staged_config_is_stale(bundle_dir: Path, fingerprint: str) -> bool:
    ...
    :func:`_config_fingerprint`) against the value recorded at generate time —
    ``config_fingerprint`` in the manifest, or (legacy) the ``.dao-ai-generated``
    marker's content. A missing or empty recorded fingerprint (a legacy dir with
    no fingerprint, or a workflow bundle) returns False: there is no recorded
    fingerprint to contradict, and :func:`_staging_dir_has_local_edits` already
    guards those dirs from an unwanted regenerate.
    """
    manifest: dict[str, Any] | None = _read_staging_manifest(bundle_dir)
    if manifest is not None:
        staged = str(manifest.get("config_fingerprint") or "").strip()
        return bool(staged) and staged != fingerprint

    marker: Path = bundle_dir / _STAGING_MARKER
    if not marker.exists():
        return False
    staged = marker.read_text().strip()
    if not staged:
        return False
...
def run_databricks_command(
    ...
    if config_path and app_config:
        ...
        if stage:
            ...
            registry = write_pipeline_bundle(
                app_config,
                staging_dir,
                overwrite=overwrite,
                development=use_local_source,
            )
            # Empty fingerprint: workflow up always re-stages, so config-drift
            # detection is moot; the file registry still guards hand-edits.
...
def _stage_app_bundle(
    ...
    *,
    ...
    registry = writer(config, bundle_dir, overwrite=overwrite, development=development)
    _write_staging_manifest(
        bundle_dir,
        is_default=is_default_dir,
