...
| `permission denied for table genie_context_aware_cache` or `permission denied for sequence genie_context_aware_cache_id_seq` at semantic-cache write. | A *prior* App SP created the cache tables and the current SP doesn't have `INSERT` + sequence `USAGE`/`UPDATE`. Note: dao-ai's cache table is **multi-tenant by design** — rows are isolated by `genie_space_id` with a `(genie_space_id, question)` UNIQUE index. Multiple Apps on the same Lakebase project are intended to share one cache table; the failure mode is a missing GRANT, not an ownership-isolation problem. | A Lakebase project owner grants the current SP the needed privileges: `GRANT INSERT, UPDATE, DELETE ON TABLE genie_context_aware_cache, genie_prompt_history TO PUBLIC; GRANT USAGE, SELECT, UPDATE ON SEQUENCE genie_context_aware_cache_id_seq, genie_prompt_history_id_seq TO PUBLIC;` Once any SP has done this, future Apps Just Work. (LangGraph's checkpointer avoids this entirely by using composite text PKs instead of `SERIAL` — a structural improvement we're tracking.) |
