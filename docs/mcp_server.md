...
| `permission denied for sequence <name>_id_seq` at semantic-cache write — even with table-level grants applied. | A *prior* deploy of the App (different name → different SP) created the cache tables and still owns them. Lakebase's `DATABRICKS_SUPERUSER` membership is `NOINHERIT`, so even `nate.fleming@databricks.com` can't override ownership or DROP the orphaned objects. | Set explicit per-deploy `table_name` + `prompt_history_table` on `context_aware_cache_parameters` (e.g. `<app_name>_cache` / `<app_name>_prompt_history`). The current App SP then creates + owns fresh tables. See `config/examples/15_complete_applications/sporting_goods_store_mcp.yaml` for the recommended pattern. |
