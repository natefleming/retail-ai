...
```mermaid
flowchart LR
    subgraph KICKOFF["Kickoff (background=true)"]
        K1[Generate response_id]
        K2[INSERT responses row<br/>status=in_progress]
        K3[asyncio.Task on _BackgroundLoop]
        K4[Return in_progress ResponsesAgentResponse]
        K1 --> K2 --> K3 --> K4
    end
    ...
    subgraph CANCEL["Cancel (operation=cancel)"]
        C1[Look up _tasks&#91;response_id&#93;]
        C2{task found<br/>on this pod?}
        C3[task.cancel&#40;&#41;<br/>best-effort]
        C4[UPDATE status=cancelled]
        C1 --> C2
        C2 -->|yes| C3 --> C4
        C2 -->|no| C4
    end

    style KICKOFF fill:#e1f5ff,stroke:#1B3139
    style RETRIEVE fill:#e1ffe1,stroke:#1B3139
    style CANCEL fill:#ffe1e1,stroke:#1B3139
```
