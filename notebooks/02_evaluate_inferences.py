...
# there, else the published PyPI package. This notebook loads a logged agent and
# runs scorers against it, so it needs every optional feature extra: ``[all]``.
# The spec is single-quoted in the magic so a dev wheel's ``+local`` version tag
# and the bracket survive shell expansion.
#
...
if not widget_path:
    raise ValueError(
        "No config to evaluate against: the `config-path` widget is empty. Set it "
        "to a config YAML — relative to this notebook (e.g. "
