...
# there, else the published PyPI package. ``[all]`` because this notebook builds
# the cache-backed agent graph. ``optuna`` is installed alongside: the tuning
# study below is the only thing in the repo that uses it, so it is not a dao-ai
# dependency. The spec is single-quoted in the magic so a dev wheel's ``+local``
# version tag and the bracket survive shell expansion.
#
...
# MAGIC %uv pip install --quiet '{_dao_ai_dep}' optuna
