...
def arma_node(model_config: ModelConfig) -> AgentCallable:
    ...
    allowable_classifications: Seqence[str] = find_allowable_classifications(catalog_name, database_name)
    ...
    def arma_question(state: AgentState, config: AgentConfig) -> dict[str, str]:
        ...
        config["allowable_classifications"] = allowable_classifications

        agent: CompiledStateGraph = create_arma_agent(model_config=model_config, config=config)
        messages: Sequence[BaseMessage] = state["messages"]
  
        input: dict[str, Sequence[BaseMessage]] = {
            "messages": messages
        }
        response = agent.invoke(input, config=config)
        logger.debug(f"response: {response}")
        last_message: AIMessage = last_ai_message(response["messages"])
