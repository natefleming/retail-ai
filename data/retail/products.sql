...
CREATE OR REPLACE TABLE products (
  ...
  ,weight DECIMAL(10,2) COMMENT 'Product weight'
  ,weight_unit STRING COMMENT 'Unit of measurement for weight (e.g., kg, lb)'
  ,dimensions VARIANT COMMENT 'Product dimensions in JSON format (length, width, height)'
  ...
  ,attributes VARIANT COMMENT 'Additional product attributes in JSON format'
  ...
  ,alternative_suppliers VARIANT COMMENT 'List of alternative suppliers in JSON format'
  
  -- Product status and lifecycle
  ,product_status STRING COMMENT 'Current product status (active, discontinued, pending)' NOT NULL
  ...
  ,is_seasonal BOOLEAN COMMENT 'Whether the product is seasonal' NOT NULL
  ...
  ,is_returnable BOOLEAN COMMENT 'Whether the product can be returned' NOT NULL
  ,return_policy STRING COMMENT 'Specific return policy for the product'
  
  -- Marketing and merchandising
  ,is_featured BOOLEAN COMMENT 'Whether the product is featured in promotions' NOT NULL
  ,promotion_eligibility BOOLEAN COMMENT 'Whether the product can be promoted' NOT NULL
  ,tags ARRAY<STRING> COMMENT 'Search and classification tags'
  ,keywords ARRAY<STRING> COMMENT 'Search keywords'
  ...
  ,hazmat_flag BOOLEAN COMMENT 'Whether product is considered hazardous material' NOT NULL
  ,regulatory_flags VARIANT COMMENT 'Regulatory compliance flags in JSON format'
  ,age_restriction INT COMMENT 'Minimum age requirement for purchase if applicable'
  ,certification_requirements VARIANT COMMENT 'Required certifications in JSON format'
  
  -- Metadata
  ,created_at TIMESTAMP COMMENT 'Record creation timestamp' NOT NULL
  ,updated_at TIMESTAMP COMMENT 'Record last update timestamp' NOT NULL
  ,created_by STRING COMMENT 'User who created the record'
  ,updated_by STRING COMMENT 'User who last updated the record'
)
CLUSTER BY AUTO
COMMENT 'Enhanced master product catalog containing comprehensive product information, including detailed attributes, inventory parameters, supplier data, and merchandising details'
TBLPROPERTIES (
