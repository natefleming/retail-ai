...
def test_agent_callable_return_type_flexibility() -> None:
    ...
    def agent_with_list_return(state: AgentState, config: AgentConfig) -> dict[str, list]:
        """Agent function that returns a list in the dict."""
        return {"messages": ["new message"]}
    
    def agent_with_mixed_return(state: AgentState, config: AgentConfig) -> dict[str, any]:
