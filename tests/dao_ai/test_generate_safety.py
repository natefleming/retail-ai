...
# ---------------------------------------------------------------------------
# The staging dir is ephemeral build output: config drift is tracked via a
# single config checksum in the manifest (no per-file edit detection).
# ---------------------------------------------------------------------------


@pytest.mark.unit
class TestConfigChecksumDriftOnly:
    def test_manifest_records_only_checksum(self, tmp_path: Path) -> None:
        import yaml

        from dao_ai.cli import _config_checksum, _write_staging_manifest

        cfg = _config_with_src(tmp_path)
        out = tmp_path / "bundle"
        write_bundle(cfg, out, overwrite=True)
        checksum = _config_checksum(cfg, development=False)
        _write_staging_manifest(out, is_default=True, checksum=checksum)

        manifest = yaml.safe_load((out / ".manifest.yaml").read_text())
        # Only the checksum is recorded — no files:/tracked: edit-detection.
        assert manifest["checksum"] == checksum
        assert "files" not in manifest
        assert "tracked" not in manifest

    def test_staleness_detects_config_change(self, tmp_path: Path) -> None:
        from dao_ai.cli import (
            _config_checksum,
            _staged_config_is_stale,
        ...
        # Same checksum → not stale (idempotent build skips).
        assert _staged_config_is_stale(out, checksum) is False
        # A different checksum (config changed) → stale → up rebuilds.
        assert _staged_config_is_stale(out, "different-checksum") is True
