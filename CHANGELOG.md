...
- **MCP server for dao-ai tools, with one-command `dao-ai generate-mcp` deploy.** Any dao-ai config that defines `create_genie_toolkit` or `create_vector_search_tool` factories can now be exposed as a Model Context Protocol (MCP) server hosted on Databricks Apps — no agent runtime, no LangGraph, just the tools served over [Streamable HTTP](https://modelcontextprotocol.io/specification/2025-03-26/basic/transports). Any MCP client (Claude Desktop, Cursor, agent platforms) can connect to a deployed app and call the tools natively.
  ...
  - **Diagnostic error on the Postgres semantic cache's "orphaned-table" failure mode.** The dao-ai semantic cache table is designed for multi-tenant use — rows are isolated by `space_id`, so multiple Genie spaces and multiple MCP/agent deployments sharing one Lakebase project should *share one cache table* (`genie_context_aware_cache` / `genie_prompt_history`). But if an App is renamed (or redeployed under a different name) the new App SP can't take ownership of the prior SP's tables: Lakebase enforces NOINHERIT on `DATABRICKS_SUPERUSER`, so even a superuser-member can't `ALTER TABLE ... OWNER` or successfully `GRANT` on the orphan's sequences. `PostgresContextAwareGenieService` now detects `InsufficientPrivilege` at table-create + write time and re-raises with a diagnostic that names the cause (orphaned cache tables in this Lakebase under a different owner) and points at the two remediation paths in `docs/mcp_server.md`: (preferred) have a Lakebase project owner `DROP` the orphan, or (emergency unblock) set per-deploy `table_name` + `prompt_history_table` so the current SP creates fresh tables it owns. End-to-end verified on FEVM: with the canonical shared-table pattern on a clean Lakebase, both cache layers fire (`served_by=...-lru` on exact match, `served_by=...-postgres-semantic` on similar phrasings).
