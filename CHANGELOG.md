...
- **MCP server for dao-ai tools, with one-command `dao-ai generate-mcp` deploy.** Any dao-ai config that defines `create_genie_toolkit` or `create_vector_search_tool` factories can now be exposed as a Model Context Protocol (MCP) server hosted on Databricks Apps — no agent runtime, no LangGraph, just the tools served over [Streamable HTTP](https://modelcontextprotocol.io/specification/2025-03-26/basic/transports). Any MCP client (Claude Desktop, Cursor, agent platforms) can connect to a deployed app and call the tools natively.
  ...
  - **Recommended per-deploy `table_name` / `prompt_history_table` on `context_aware_cache_parameters`** — set them to `<app_name>_cache` / `<app_name>_prompt_history` (rather than the shared dao-ai defaults `genie_context_aware_cache` / `genie_prompt_history`) so renaming or redeploying the App doesn't orphan the prior deploy's tables. Lakebase's strict NOINHERIT on `DATABRICKS_SUPERUSER` means no role outside the original App SP can take over orphaned tables, so the semantic cache silently no-ops until you point at fresh names. Verified end-to-end on FEVM: `served_by=merchandising_analytics-postgres-semantic` lit up immediately after switching to unique table names. Docs and the sporting-goods example reflect the pattern.
