...
- **Inbound WhatsApp channel via the Meta Cloud API.** End users can now chat with a dao-ai agent over WhatsApp. When `app.channels.whatsapp` is set on a Databricks Apps deployment, two FastAPI routes are mounted on the existing app: `GET /channels/whatsapp/webhook` (Meta verification handshake) and `POST /channels/whatsapp/webhook` (inbound delivery). The POST handler verifies the `X-Hub-Signature-256` HMAC (constant-time `hmac.compare_digest` against the raw body + Meta App Secret), dedupes on Meta's message id via a Lakebase `UNIQUE` constraint, returns 200 in <20 s, and dispatches the agent in the background. Background dispatch reuses `dao_ai.apps.handlers.non_streaming` so every guardrail, middleware, OBO header injection, memory, MLflow trace, and long-running offloading the agent already has applies unchanged. Replies are POSTed to `graph.facebook.com/{version}/{phone_number_id}/messages` with paragraph/sentence-aware chunking for WhatsApp's 4096-char hard limit.

  Highlights:
  - **New config block** `app.channels.whatsapp` (`WhatsAppChannelModel` / `ChannelsModel`) layered next to `app.long_running` and `app.a2a`. All three Meta credentials (`verify_token`, `app_secret`, `access_token`) are typed `SecretVariableModel` and resolve at request time, so `databricks secrets put-secret` takes effect without an app restart. Field-level validators reject `static` strategies without a `static_thread_id` and `webhook_path` values that don't start with `/`.
  - **Lakebase-backed `ChannelStore`** (`dao_ai_whatsapp_inbound_dedup` with UNIQUE `message_id`; `dao_ai_whatsapp_threads` mapping `wa_id`→`thread_id`) created idempotently on first use, with an in-memory fallback for tests. The connection pool is shared with the LangGraph checkpointer and `LongRunningStore` when the same `DatabaseModel` is referenced.
  - **Thread continuity** — three strategies: `wa_id` (one thread per sender, default), `wa_id+phone_number_id` (per-business-number scope), `static` (debug). The resolved `thread_id` flows through `custom_inputs.configurable` into LangGraph's checkpointer, so existing memory examples (`config/examples/05_memory/`) work unchanged for WhatsApp conversations.
  - **PII redaction** — `wa_id` is SHA-256-hashed before being emitted as an MLflow trace attribute (`redact_phone_in_traces=true` by default).
  - **Companion outbound tool** — `dao_ai.tools.whatsapp.create_send_whatsapp_message_tool` mirrors the slack/teams tool factories; uses a Unity Catalog HTTP Connection to `graph.facebook.com` so credentials live in UC instead of env vars. Outside Meta's 24-hour customer service window, this tool requires a pre-approved template (a Meta Business Manager workflow, not a dao-ai concern).
  - **Server-side wiring** — `_mount_channel_routes()` added to `dao_ai.apps.server` next to `_mount_long_running_routes()` and `_mount_a2a_routes()`. Same defensive try/except pattern; no-op when `app.channels` is unset.
  - **Example config** `config/examples/21_channels/whatsapp.yaml` — runnable end-to-end with `app.channels.whatsapp`, a Lakebase database shared with the checkpointer, and inline pointers to the three Databricks secret-scope keys you need to create.
  - **26 unit tests** in `tests/dao_ai/apps/channels/test_whatsapp.py` covering signature verification (positive + tampered body + wrong secret + missing-prefix + empty header), text chunking (under-limit + paragraph-preferred split + hard-wrap), inbound message parsing (text + image/audio dropped + interactive button reply), thread key strategies, config validators, MLflow `OutputItem`-shaped reply extraction, in-memory dedup, and the full FastAPI route surface via TestClient including the Meta-redelivery dedup path.
  - **End-to-end verified on FEVM** (`fevm-nfleming-stable-aws`): App `whatsapp-test-nf` deployed via Databricks Asset Bundle. GET `?hub.mode=subscribe&hub.verify_token=<correct>&hub.challenge=ping123` returns 200 + body `ping123`; GET with wrong token returns 403; POST with tampered `X-Hub-Signature-256` returns 403; POST with valid HMAC-SHA256 returns 200 + `{"status":"received"}`.
  - **Docs.** New `docs/whatsapp_channel.md` covers the wire shape, request lifecycle, full config reference, production-readiness checklist, end-to-end test recipe (Meta side + Databricks side + curl sanity checks), and troubleshooting (Apps public-access gotcha, secret-rotation semantics, `enable_chat_proxy` 502 cause, 24-hour window).
